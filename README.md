הערה: אמנם ההנחיות אמרו ליצור TAG לכל שלב בנפרד, אבל שכחנו לעשות את זה לכמה מהשלבים ועכשיו אנחנו לא מצליחים ליצור תגים לגרסאות קודמות של הפרוייקט. אנחנו מקווים שזו לא בעיה גדולה מדי. כל התיקיות והדוחות נמצאים בגרסה הזאת.
...
1.  **כל סרטי האקשן, ההרפתקאות והמותחן שיצאו לפני שנת 2000** - בשביל המערכת שרוצה להמליץ לצופים מבוגרים על סרטים נוסטלגיים שהם יאהבו
    ![image](https://github.com/user-attachments/assets/f2097d12-0eb0-4ae0-b8ff-d7c433101aa3)

2.  **קיבוץ סרטים לפי פרסים** - עבור המלצות לאנשים שהמערכת מזהה שמעריכים דברים ספציפיים בסרטים
    ![image](https://github.com/user-attachments/assets/825761b0-a56f-4a38-999f-7dbc53229c90)

3.  **סדרות עם אורך פרק ממוצע מעל 50 דקות** - סינון בשביל אנשים שאין להם הפסקות כאלו ארוכות
    ![image](https://github.com/user-attachments/assets/feb199ef-7932-40f7-bd41-7a0cf2bea6e1)

4.  **סדרות עם זמן שידור כולל מתחת ל־5 שעות** - סדרות שיהיו מעולות בשבל בינג' מהיר
    ![image](https://github.com/user-attachments/assets/4a9108e5-431f-4288-b117-b881f5cf329f)

5.  **איתור סרטי המשך עם פער זמן גדול מהסרט המקורי** - סינון טוב בשביל אנשים שלא אוהבים את תרבות הרימייקים של חברות כמו דיסני
    ![Screenshot 2025-06-18 121604](https://github.com/user-attachments/assets/d3656337-2220-4fc9-b3ad-651932346adc)

6.  **הצגת מספר הכותרים, הסרטים והסדרות בכל פרנצ'ייז** - בשביל אנשים רוצים להיכנס לפרנצ'ייז אבל לא יודעים כמה תוכן יש בו
    ![image](https://github.com/user-attachments/assets/4df46d2c-9a10-484d-aae7-81ad4c11cd23)

7.  **הסרט שזכה במרב הפרסים בכל ז'אנר** - בשביל אנשים שמחפשים את הטוב ביותר
    ![image](https://github.com/user-attachments/assets/ebda9a3d-56cb-44fe-b8f9-7eec047b3413)

8.  **כותרים מקוריים ומספר פרסים** - שוב לאנשים שמחפשים סרטים מקוריים טובים
...
**תיאור המבט**: מרכז מידע על סרטים, כולל זיהוי הז'אנר, משך הזמן, תאריך יציאה ומספר הפרסים. מיועד לצרכנים ולמפיקים.

![image](https://github.com/user-attachments/assets/8a30a633-9d9c-4ddd-9270-f12e016dd727)

**שאילתה 1.1: סרטי אקשן ארוכים שזכו בפרסים** - מפיק עשוי לרצות להשתמש בשאילתה כזו כדי להחליט מה האורך המומלץ לסרט.
![image](https://github.com/user-attachments/assets/ba1c9c00-3d43-4557-a73f-acc23fe30425)

**שאילתה 2.1: סטטיסטיקה של ז'אנרים לפי משך זמן ממוצע** - לצרכנים עם זמן מוגבל.
...
**תיאור המבט**: משלב מידע אישי על יוצרי תוכן עם נתוני סוכנים, חוזים, תשלומים ופרסים אישיים. מיועד לחברות הפקה.

![Screenshot 2025-06-25 164632](https://github.com/user-attachments/assets/ae18070b-fae8-4f5d-ad20-5425969f7d3f)

**שאילתה 1.2: יוצרי תוכן מנוסים עם תשלום גבוה מהממוצע** - שימושי בשביל החלטות תקציביות.
![image](https://github.com/user-attachments/assets/39fedb90-09eb-4ddf-bbe4-b4a54a4746a1)

**שאילתה 2.2: השוואת ביצועי יוצרים בין מדינות** - עשוי להיות שימושי בהחלטות שיווק גלובליות.
...
**תיאור מילולי**: פונקציה זו מחזירה סטטיסטיקות מפורטות עבור יוצר תוכן נתון. הפונקציה מקבלת מזהה יוצר ומחזירה מצביע לתוצאה (REF CURSOR) הכולל את השם המלא של היוצר, המדינה, מספר החוזים הכולל וממוצע התשלומים. מיועד למפיקים שמעוניינים במי לשכור.
...
**תיאור מילולי**: פונקציה זו מעדכנת אוטומטית את מספר העונות בטבלת tv_show על סמך מספר העונות הקיימות בפועל בטבלת season. מיועד למנהלי בסיס הנתונים כדי לשמות לע נתונים עקביים.
...
**תיאור מילולי**: פרוצדורה זו מבצעת פעולות ניהול על פרנצ'יז נתון. כרגע מממשת פעולת COUNT שסופרת ומעדכנת את מספר הכותרים השייכים לפרנצ'יז. מיועד למנהלי בסיס הנתונים.
...
**תיאור מילולי**: פרוצדורה זו עוברת על כל העונות במערכת ומעדכנת את הסטטוס שלהן בהתאם למספר הפרקים הקיימים. למנהלי בסיס הנתונים.
...
**תיאור מילולי**: טריגר זה מופעל אוטומטית בכל פעם שמתווסף או נמחק כותר מפרנצ'יז (טבלת belongs_to). הטריגר מעדכן אוטומטי את השדה number_of_titles בטבלת franchise. עוזרת לשמור על מידע עקבי.
...
**תיאור מילולי**: טריגר זה מופעל לפני הוספה או עדכון של חוזה ובודק את תקינות הנתונים. עוזרת לשמור על מידע עקבי.
